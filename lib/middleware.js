...
async function authMiddleware(req, res, next) {
  ...
  if (req.path === '/users/cambiarEstado') return next()
  if (req.path === '/users/mandadosCliente') return next()
  if (req.path === '/users/actualizarUbicacion') return next()
  if (req.path === '/users/getRepartidor') return next()
